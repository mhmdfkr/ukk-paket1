...
class Auth extends CI_Controller
{
  ...
  public function registration()
  {
    ...
    $this->form_validation->set_rules('nik', 'Nik', 'trim|required|max_length[16]', [
      'max_length' => 'The maximum of length is 16!'
    ]);
  ...
  private function _sendEmail($token, $type)
  {
    $config = [
      ...
      'smtp_user' => 'mhmdfkr0415@gmail.com',
      'smtp_pass' => '0415mhmdfkr',
      'smtp_port' => 465,
      'mailtype'  => 'html',
      'charset'   => 'utf-8',
      'newline'   => "\r\n"
    ...
    $this->email->from('mhmdfkr0415@gmail.com', 'MUHAMAD FIKRI');
    $this->email->to($this->input->post('email'));
    if ($type == 'verify') {
      $this->email->subject('Account Verification');
      $this->email->message('Click this link to verify you account : <a href="' . base_url() . 'auth/verify?email=' .
        $this->input->post('email') . '&token=' .  urlencode($token) . '">Activate</a>');
    } else if ($type == 'forgot') {
      $this->email->subject('Reset Password');
      $this->email->message('Click this link to reset password : <a href="' . base_url() . 'auth/resetpassword?email=' .
